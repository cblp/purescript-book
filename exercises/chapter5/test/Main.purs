...
import ChapterExamples
  ( Amp(..), current, fromString, gcd, gcdV2, isEmpty, livesInLA, lzs
  , partialFunction, showPerson, showPersonV2, sortPair, takeFive, toString
  , unknownPerson, Volt(..)
  )
import Data.Person (Person)
import Data.Picture (bounds, getCenter, origin, Picture, Shape(..), shapeBounds)
...
john =
  { name: "John Smith"
  , address: { street: "123 Test Lane", city: "Los Angeles" }
  }

rose :: Person
rose =
  { name: "Rose Jackson"
  , address: { street: "464 Sample Terrace", city: "Los Angeles" }
  }

amy :: Person
amy =
  { name: "Amy Lopez", address: { street: "10 Purs Street", city: "Omaha" } }
...
main =
  runTest do
    ...
    suite "Exercise Group - Simple Pattern Matching" do
      ...
      suite "Exercise - binomial" do
        test "10 0" $ Assert.equal   1 $ binomial 10 0
        test " 0 3" $ Assert.equal   0 $ binomial  0 3
        test " 2 5" $ Assert.equal   0 $ binomial  2 5
        test "10 5" $ Assert.equal 252 $ binomial 10 5
        test " 5 5" $ Assert.equal   1 $ binomial  5 5
      suite "Exercise - pascal" do
        test "10 0" $ Assert.equal   1 $ pascal 10 0
        test " 0 3" $ Assert.equal   0 $ pascal  0 3
        test " 2 5" $ Assert.equal   0 $ pascal  2 5
        test "10 5" $ Assert.equal 252 $ pascal 10 5
        test " 5 5" $ Assert.equal   1 $ pascal  5 5
    suite "Exercise Group - Array and Record Patterns" do
      test "Exercise - sameCity" do
        Assert.equal true  $ sameCity john rose
        Assert.equal false $ sameCity amy  rose
      test "Exercise - fromSingleton" do
        Assert.equal "default" $ fromSingleton "default" []
        Assert.equal "B"       $ fromSingleton "default" ["B"]
        Assert.equal "default" $ fromSingleton "default" ["B", "C", "D"]
    suite "Exercise Group - Algebraic Data Types" do
      test "Exercise - circleAtOrigin" do
        Assert.equal origin $ getCenter circleAtOrigin
      ...
      test "Exercise - shapeText" do
        ...
        Assert.equal Nothing $ shapeText $ Circle origin 1.0
        Assert.equal Nothing $ shapeText $ Rectangle origin 1.0 1.0
        Assert.equal Nothing $ shapeText $ Line origin { x: 1.0, y: 1.0 }
    suite "Exercise Group - Newtype" do
      test "Exercise - calculateWattage" do
        Assert.equal 60.0
          $ let (Watt w) = calculateWattage (Amp 0.5) (Volt 120.0) in w
    suite "Exercise Group - Vector Graphics" do
      test "Exercise - area" do
        Assert.equal 50 $ round $ area $ Circle origin 4.0
        Assert.equal 40 $ round $ area $ Rectangle origin 4.0 10.0
        Assert.equal  0 $ round $ area $ Line origin { x: 2.0, y: 2.0 }
        Assert.equal  0 $ round $ area $ Text origin "Text has no area!"
      suite "Exercise - Clipped shapeBounds" do
        test "samplePicture"
          $ Assert.equal { top: -2.0, left: -2.0, right: 7.0, bottom: 7.0 }
          $ bounds samplePicture
        test "Clipped 1"
